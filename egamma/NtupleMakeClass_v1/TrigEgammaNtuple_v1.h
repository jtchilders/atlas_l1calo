#ifndef TrigEgammaNtuple_v1_h
#define TrigEgammaNtuple_v1_h
...
class TrigEgammaNtuple_v1 {
  public :
   ...
   map<string,bool> *Trigger_L1passed;
   map<string,bool> *Trigger_L2passed;
   map<string,bool> *Trigger_EFpassed;
   ...
   vector<float>   *TrigElectron_trkEtaAtCalo; // track extrapolation to calo RoI center of gravity in LVL2
   ...
   vector<int>     *TrigInDetTrack_AlgoId; // SiTrack = 1; IdScan = 2; TRT = 3
   vector<float>   *TrigInDetTrack_chi2;
   vector<string>  *TrigInDetTrack_slice; // container name
   vector<int>     *TrigInDetTrack_NStrawHits; // # TRT hits associated with track
   vector<int>     *TrigInDetTrack_NStraw; // # TRT straws intersected by track
   vector<int>     *TrigInDetTrack_NStrawTime; // # TRT straws with valid drift time intersected by track
   vector<int>     *TrigInDetTrack_NTRHits; // # TRT high threshold hits associated with track
   vector<int>     *TrigInDetTrack_NSiSpacePoints;
   vector<int>     *TrigInDetTrack_BL_hits;
   vector<int>     *TrigInDetTrack_Pixel_hits; // # pixel space points associated with track
   vector<int>     *TrigInDetTrack_SCT_hits; // # SCT space points associated with track
   ...
   vector<int>     *EFegamma_TrackIndex; // -1 if no track for this egamma object, index = vector entry number
   ...
   TrigEgammaNtuple_v1(TFile *file, std::string outFile);
   
   //TrigEgammaNtuple_v1(TFile *file);
   virtual ~TrigEgammaNtuple_v1();
   ...
   bool EventPassedTrig(map<string,bool> *list, std::string trig);
   bool ObjInTrigger(map<string,bool> list, std::string trig);
   bool ObjPassedTrigger(map<string,bool> list, std::string trig);
};
#endif

#ifdef TrigEgammaNtuple_v1_cxx
TrigEgammaNtuple_v1::TrigEgammaNtuple_v1(TFile *file, std::string outFile)
{
...
void TrigEgammaNtuple_v1::SetnEntries(Long64_t en) 
{
  nEntry=en;
}

void TrigEgammaNtuple_v1::addFile(TFile *f)
{
  if (f!=0) {
    TDirectory* dir=0;
...
TrigEgammaNtuple_v1::~TrigEgammaNtuple_v1()
{
  // ?????
}

Bool_t TrigEgammaNtuple_v1::CheckTrees() 
{
  if (fChainInfo || fChainTrigger || fChainL1 || fChainL2Cluster || fChainL2Electron 
      || fChainL2Photon || fChainL2Track || fChainEFCluster || fChainEFegamma ||
      fChainEFTrack || fChainOFFCluster || fChainOFFegamma || fChainOFFTrack) return  kTRUE;
  else return kFALSE;
}


Long64_t  TrigEgammaNtuple_v1::LoadTree(Long64_t entry)
{
  Long64_t centryInfo=-9999;
  if (hasInfo) {
    centryInfo=fChainInfo->LoadTree(entry);
    fChainInfo->GetEntry(entry);
  }
  
  Long64_t centryTruth=-9999;
  if (hasTruth) {
    centryTruth=fChainTruth->LoadTree(entry);
    fChainTruth->GetEntry(entry);
  }
  
  Long64_t centryTrigger=-9999;
  if (hasTrigger) {
    centryTrigger=fChainInfo->LoadTree(entry);
    fChainTrigger->GetEntry(entry);
  }

  Long64_t centryL1=-9999;
  if (hasL1) {
    centryL1=fChainL1->LoadTree(entry);
    fChainL1->GetEntry(entry);
  ...
  if (hasL2Cluster) {
    ...
    fChainL2Cluster->GetEntry(entry);
  ...
  if (hasL2Electron) {
    ...
    fChainL2Electron->GetEntry(entry);
    if (!hasL2Cluster) {
  ...
  if (hasEFCluster) {
    ...
    fChainEFCluster->GetEntry(entry);
...
Int_t TrigEgammaNtuple_v1::GetEntries()
{
  Int_t entryInfo=-10;
  if (hasInfo) {
    if (!fChainInfo) {
...
{ 
  ...
  //Trigger Info Leaf
  Trigger_L1passed = new map<string,bool>();
  Trigger_L2passed = new map<string,bool>();
  Trigger_EFpassed = new map<string,bool>();
...
void TrigEgammaNtuple_v1::FillTriggerMap(vector<vector<string> >* names,  vector<vector<int> >* dec,vector<map<string,bool> >& omap) {
  omap.clear();
  if (names!=0) {
...
bool TrigEgammaNtuple_v1::EventPassedTrig(map<string,bool> *list, std::string trig) {
...
bool TrigEgammaNtuple_v1::ObjInTrigger(map<string,bool> list, std::string trig) {
  std::map<string,bool>::iterator it=list.find(trig);
  if (it!=list.end()) {
    return true;
  }
...
bool TrigEgammaNtuple_v1::ObjPassedTrigger(map<string,bool> list, std::string trig) {
  std::map<string,bool>::iterator it=list.find(trig);
  if (it!=list.end()) {
    if (it->second) return true;
  }
