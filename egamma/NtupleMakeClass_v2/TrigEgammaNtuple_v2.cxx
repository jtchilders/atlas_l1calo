#define TrigEgammaNtuple_v2_cxx
...
void TrigEgammaNtuple_v2::DumpTrigger(string level, bool verbose=false) {
  cout << endl << "Trigger information for the event: " << Event_evt_number << endl;
  if ( level.find("L1")!=string::npos || level=="ALL") {
    cout << "** L1 chains: " << endl;
    for (unsigned int i=0; i<Trigger_L1ChainName->size(); i++) {
      if ( Trigger_L1passed->at(i) )            cout << "   - " << Trigger_L1ChainName->at(i) << " is PASSED" << endl;
      if (verbose && !Trigger_L1passed->at(i) ) cout << "   - " << Trigger_L1ChainName->at(i) << " is NOT passed" << endl;
    }
  }
  if ( level.find("L2")!=string::npos || level=="ALL") {
    cout << "** L2 chains: " << endl;
    for (unsigned int i=0; i<Trigger_L2ChainName->size(); i++) {
      if ( Trigger_L2passed->at(i) )            cout << "   - " << Trigger_L2ChainName->at(i) << " is PASSED" << endl;
      if (verbose && !Trigger_L2passed->at(i) ) cout << "   - " << Trigger_L2ChainName->at(i) << " is NOT passed" << endl;
    }
  }
  if ( level.find("EF")!=string::npos || level=="ALL") {
    cout << "** EF chains: " << endl;
    for (unsigned int i=0; i<Trigger_EFChainName->size(); i++) {
      if ( Trigger_EFpassed->at(i) )            cout << "   - " << Trigger_EFChainName->at(i) << " is PASSED" << endl;
      if (verbose && !Trigger_EFpassed->at(i) ) cout << "   - " << Trigger_EFChainName->at(i) << " is NOT passed" << endl;
    }
  }
}


void TrigEgammaNtuple_v2::DumpL1(unsigned int i, bool verbose=false) {
...
void TrigEgammaNtuple_v2::DumpL2Cluster(unsigned int i, bool verbose=false) {
...
void TrigEgammaNtuple_v2::DumpL2Photon(unsigned int i, bool verbose=false) {
...
void TrigEgammaNtuple_v2::DumpL2Track(unsigned int i, bool verbose=false) {
...
void TrigEgammaNtuple_v2::DumpL2Electron(unsigned int i, bool verbose=false) {
...
void TrigEgammaNtuple_v2::DumpCluster(unsigned int i, bool off=false, bool verbose=false) {
  if ( off ) {
...
void TrigEgammaNtuple_v2::DumpEgamma(unsigned int i, bool off=false, bool verbose=false) {
  if (off) {
...
void TrigEgammaNtuple_v2::Loop() {
  ...
  */
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    if ( jentry==0 ) syncro=0;
    Long64_t ientry = LoadTree(jentry); 
    if (ientry < 0) break;
    if (ientry==0) { 
      syncro++;
      Long64_t  test1 = fChainMetadata->LoadTree(syncro);
      Long64_t  test2 = fChainMetadata->GetEntry(syncro);
    ...
    */
    ...
    nEvent++;
    cout << "Namesize: " << Trigger_L1ChainName->size() << " L1passed: " << Trigger_L1passed->size() << " L1passedPT: " << Trigger_L1passed_PT->size() << " prescale: " <<  Trigger_L1prescale->size() << endl;
    cout << "Namesize: " << Trigger_L2ChainName->size() << " L2passed: " << Trigger_L2passed->size() << " L2passedPT: " << Trigger_L2passed_PT->size() << " prescale: " <<  Trigger_L2prescale->size() << endl;
    cout << "Namesize: " << Trigger_EFChainName->size() << " EFpassed: " << Trigger_EFpassed->size() << " EFpassedPT: " << Trigger_EFpassed_PT->size() << " prescale: " <<  Trigger_EFprescale->size() << endl;
    cout << endl;
